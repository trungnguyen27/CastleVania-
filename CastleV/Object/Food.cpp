...
Food::Food(int x, int y) : BaseObject(FOOD)
{
	_sprite = SpriteManager::getInstance()->getSprite(eID::ITEM);
	_sprite->setFrameRect(SpriteManager::getInstance()->getSourceRect(eID::ITEM, "food"));
	_sprite->setPosition(x, y);
	_initX = x;
	_stop = false;
}

void Food::draw(LPD3DXSPRITE spriteHandle, Viewport* viewport)
{
...
RECT Food::getBounding()
{
	return _sprite->getBounding();
}


float Food::checkCollision(BaseObject* object, float dt)
{
